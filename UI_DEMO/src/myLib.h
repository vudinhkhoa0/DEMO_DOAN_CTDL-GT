...
namespace NS_myLib {
    ...
    std::string check_planeCode (std::string planeCode) {
        ...
        if (planeCode.length() > 8) return "Vượt quá 8 ký tự";
        // kiểm tra chữ đầu
        if (planeCode.length() != 0)
            if (!check_character_lowercaseLetter(planeCode[0]) && !check_character_uppercaseLetter(planeCode[0])) return "Ký tự đầu không phải số";
        // kiểm tra ký tự đặc biệt
        for (int i = 0; i < planeCode.length(); i++) {
            if (!check_character(planeCode[i])) return "Ký tự đặc biệt";
        }
        return "True";
    }
    // -- //
    ...
    string ticket_checkInputFirstName(string str) {
        ...
        for (int i = 0; i < str.length(); ++i) {
            if ((str[i] < 'A' || str[i] > 'Z') && str[i] != ' ') return "Nhap sai";
        }
        return "";
    }
    string ticket_checkInputLastName(string str) {
        ...
        for (int i = 0; i < str.length(); ++i) {
            if ((str[i] < 'A' || str[i] > 'Z')) return "Nhap sai";
        }
        return "";
    }
    // -- //
